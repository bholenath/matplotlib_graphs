...
# color_arr = []
f, ax = plt.subplots(1, 1, figsize=(1600 / 95.8, 900 / 95.8), dpi=96)
...
post_addition = np.linspace(-0.4,0.4,number_aspects-12)
color_arr = cmx.rainbow(np.linspace(0, 1, number_aspects - 12))

while index < number_aspects - 12:
    ...
    ax.bar(college_arr + post_addition[index], valid_show_aspects, width=0.1, color=color_arr[index], align='center',
           label=aspects_value[index],rasterized=True,antialiased=True)
    index += 1
...
ax.set_xticklabels(college_name, fontdict='monospace', zorder=1, color='blue', rotation='20',
                   va='top', ha='center', weight='normal', x=0.001, y=0)

ax.tick_params(axis='both', width=1.5, direction='in')
ax.margins(x=0.001, tight=True)
# ax.tick_params(axis='x', width=8, direction='out')
...
# color_arr = ['red', 'green', 'blue', 'yellow','black','grey']
...
# college_name_dd_array = [np.arange(1, length + 1, 1) for _ in range(length_aspects_list)]
...
# axes_plot = ax.plot(aspects_dd_list, 'o', color=color_cycle, markersize=6, antialiased=True,
#                     rasterized=True)
...
plt.grid()
plt.show()
img_name = "graph_stackplot_aspects_bar.png"
f.savefig(img_name, dpi=95.6)
