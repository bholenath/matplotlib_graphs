...
f.suptitle('Colleges_Correlation_between_Sentiment_&_Overall_Score(all colleges tweets)', style='italic', family='monospace', weight='bold', size='large',
           ha='center', va='top')
...
median_col = np.mean(np.array(list(set(overall_college_score))))
# mid_y_val = min(sentiment_score) + (abs(min(sentiment_score)) + max(sentiment_score))/2
mid_y_val = 0.0
# mid_x_val = - 0.5 + median_col

# print str(loc_sen), "", min(sentiment_score), "", str(mid_y_val)
...
i = 0
for sen, col in zip(sentiment_score, overall_college_score):
    if sen <= mid_y_val and col <= median_col:
        # col_arr.append('red')
        grid_1_sen.append(sen)
        grid_1_col.append(col)
        count_grid_1_college += 1
        college_1_grid.append(college_name[i])
        college_rows_1_grid.append(tweets_analyzed[i])
        ...
        college_2_grid.append(college_name[i])
        college_rows_2_grid.append(tweets_analyzed[i])
        ...
        college_3_grid.append(college_name[i])
        college_rows_3_grid.append(tweets_analyzed[i])
        # labels.append('Expected Values')
    elif sen > mid_y_val and col <= median_col:
        # col_arr.append('violet')
        ...
        college_4_grid.append(college_name[i])
        college_rows_4_grid.append(tweets_analyzed[i])
        # labels.append('UNEXPECTED VALUES')
    i += 1

loc_col = plticker.MultipleLocator(base=(median_col + 0.02))
# val_tick = (abs(min(sentiment_score)) + max(sentiment_score))/2
# locs = dict(vmin=val_tick,vmax=val_tick)
# val_tick = mid_y_val
loc_sen = plticker.FixedLocator(np.linspace((mid_y_val + 0.02), (mid_y_val + 0.02), 1))
# print (abs(min(sentiment_score)) + max(sentiment_score))/2
...
if grid_4_sen:
    ...
    # ax[0][0].set_yticklabels([''+str((abs(min(sentiment_score)) + max(sentiment_score))/2)],size='small',minor=True,visible=False)
    ...
    for j, item in enumerate(grid_3_sen):
        if grid_3_col[j] >= ((max(grid_2_col + grid_3_col) - median_col) / 1.3) + median_col and grid_3_sen[j] >= (
                    0 + max(grid_3_sen + grid_4_sen)) / 1.3 or grid_3_col[j] <= ((max(grid_2_col + grid_3_col) - median_col) / 1.3) + median_col and \
                        grid_3_sen[
                            j] >= (0 + max(grid_3_sen + grid_4_sen)) / 1.1:
...
# ax[0][1].set_yticks(np.linspace(min(grid_1_sen+grid_2_sen)-0.1,max(grid_1_sen+grid_2_sen)+0.1,20),minor=True)
# print min(sentiment_score), min(grid_1_sen+grid_2_sen)
if grid_2_sen:
    ...
    for j, item in enumerate(grid_2_sen):
        if grid_2_col[j] >= ((max(grid_2_col + grid_3_col) - median_col) / 1.3) + median_col and grid_2_sen[j] <= (
                    0 + min(grid_1_sen + grid_2_sen)) / 1.3 or grid_2_col[j] <= ((max(grid_2_col + grid_3_col) - median_col) / 1.3) + median_col and \
                        grid_2_sen[
                            j] <= (0 + min(grid_1_sen + grid_2_sen)) / 1.3 or grid_2_col[j] >= (
                    (max(grid_2_col + grid_3_col) - median_col) / 1.5) + median_col and \
                        grid_2_sen[j] >= (0 + min(grid_1_sen + grid_2_sen)) / 1.3:
...
# plt.grid()
# ax.legend(loc=0)
print "Colleges in lower left grid : \n"
for i, val in enumerate(college_1_grid):
    print val, " - ", college_rows_1_grid[i]
    print "\n"

print "Colleges in lower right grid : \n"
for i, val in enumerate(college_2_grid):
    print val, " - ", college_rows_2_grid[i]
    print "\n"

print "Colleges in upper right grid : \n"
for i, val in enumerate(college_3_grid):
    print val, " - ", college_rows_3_grid[i]
    print "\n"

print "Colleges in upper left grid : \n",
for i, val in enumerate(college_4_grid):
    print val, " - ", college_rows_4_grid[i]
    print "\n"

text = "Total Colleges : " + str(len(sentiment_score)) + ", Total Avg. Sen : " + str(round(np.mean(np.array(sentiment_score)), 2))
f.text(0.01, 0.95, text, fontsize=14, bbox=dict(facecolor='grey', alpha=0.5, pad=7.0), weight='bold')

plt.show()
# f.tight_layout()
img_name = 'Colleges_Correlation_between_Sentiment_&_Overall_Score(all colleges tweets).svg'
f.savefig('test_' + img_name, format='svg', dpi=600)
pp = PdfPages('test_Colleges_Correlation_between_Sentiment_&_Overall_Score(all colleges tweets).pdf')
pp.savefig(f)
pp.close()
