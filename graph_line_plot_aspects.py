...
# all_aspect_check = 0
# np.arange(1,len(num))
# while all_aspec_check < number_aspects:
last = 0
while last < number_aspects:
    last += 3
    aspects_dd_list = []
    while index < last and index < number_aspects:
    ...
    ax.set_title('Colleges_Aspects_Plot_Chart', y=1.02, style='italic', family='monospace', weight='bold')
    ...
    ax.yaxis.set_major_locator(MaxNLocator(nbins=30, prune='lower'))

    ax.set_xticklabels(college_name, fontdict='monospace', zorder=1, color='blue', rotation='20',
                       va='top', ha='center', weight='roman', x=0.015, y=0, clip_on=False, axes=ax)

    ax.tick_params(axis='both', width=1.5, direction='in')
    ax.margins(x=0.015, tight=True, pad=2)

    ax.xaxis.set_major_locator(MaxNLocator(nbins=len(college_name), prune='lower'))
    ...
    # print aspects_dd_list
    # color_norm = colors.Normalize(vmin=0, vmax=length_aspects_list - 1)
    # print color_norm
    # scalar_map = cmx.ScalarMappable(norm=color_norm, cmap='hsv')
    # print scalar_map
    # color_arr = scalar_map.to_rgba([n for n in range(length_aspects_list)])
    ...
    # print labels
    # rects = ax.stackplot(np.arange(1,length+1,1),aspects_dd_list,baseline='zero',colors=color_arr)

    # side_arr_cycle = cycle([3, 4, 5, 6])
    # linestyle_arr = cycle(['-']) # can add other linestyles as well (-.)
    ...
    for i in range(len(aspects_dd_list)):
        ...
        axes_plot = ax.plot(college_name_dd_array[i], aspects_dd_list[i], color=color_cycle, markersize=4, antialiased=True,rasterized=True, label=aspects_value[i+last], linewidth=1, alpha=0.7, linestyle='-', marker='o')

        # axes_plot = ax.plot(college_name_dd_array[i], aspects_dd_list[i], color=color_cycle, markersize=6 + i, antialiased=True,rasterized=True, label=aspects_value[i], linewidth=1 + (i / 2.5), alpha=transparency.next(),        linestyle=linestyle_arr.next(), marker=(side_arr_cycle.next(), i % 4, angle_val[i]))
        #
    ...
    ax.grid(which='major', color='grey', linestyle='--', linewidth=1,alpha=0.8)
    ax.legend(loc=0)
    ...
    #     height = max_values[i] - 2.0
    #     plt.text(fig_object.get_figwidth() / 2, height, '%s' % aspects_value[i],ha='center', va='top', rotation='horizontal', weight='normal', size='medium')
    ...
    # plt.grid()
    plt.show()
    img_name = "graph_aspects_plot.svg"
    f.savefig('./college_aspects_variabilty/'+img_name, dpi=600)
    pp = PdfPages("graph_aspects_plot.pdf")
    pp.savefig(f)
    pp.close()
    plt.close(f)
